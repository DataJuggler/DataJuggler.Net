...
namespace DataJuggler.Net.Xml.Writers
{
    ...
    public class CheckConstraintsWriter
    {
        ...
        #region Methods
            ...
            #region ExportList(List<CheckConstraint> checkConstraints, int indent = 0)
            // <Summary>
            // This method is used to export a list of 'CheckConstraint' objects to xml
            // </Summary>
            public string ExportList(List<CheckConstraint> checkConstraints, int indent = 0)
            {
                // initial value
                string xml = "";

                // locals
                string checkConstraintsXml = String.Empty;
                ...
                {
                    // Iterate the checkConstraints collection
                    foreach (CheckConstraint checkConstraint  in checkConstraints)
                    {
                        // Get the xml for this checkConstraints
                        checkConstraintsXml = ExportCheckConstraint(checkConstraint, indent + 2);

                        // If the checkConstraintsXml string exists
                        if (TextHelper.Exists(checkConstraintsXml))
                        {
                            // Add this checkConstraints to the xml
                            sb.Append(checkConstraintsXml);
                        }
                    }
                }
