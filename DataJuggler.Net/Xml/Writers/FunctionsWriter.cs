...
namespace DataJuggler.Net.Xml.Writers
{
    ...
    public class FunctionsWriter
    {
        ...
        #region Methods
            ...
            #region ExportList(List<Function> functions, int indent = 0)
            // <Summary>
            // This method is used to export a list of 'Function' objects to xml
            // </Summary>
            public string ExportList(List<Function> functions, int indent = 0)
            {
                // initial value
                string xml = "";

                // locals
                string functionsXml = String.Empty;
                ...
                {
                    // Iterate the functions collection
                    foreach (Function function  in functions)
                    {
                        // Get the xml for this functions
                        functionsXml = ExportFunction(function, indent + 2);

                        // If the functionsXml string exists
                        if (TextHelper.Exists(functionsXml))
                        {
                            // Add this functions to the xml
                            sb.Append(functionsXml);
                        }
                    }
                }
